...
def sample(
    ...
    for input_img_path in all_img_paths:
        if "sv3d" in version:
            ...
            # Calculate correct padding dimensions
            top = center_y - h // 2
            bottom = top + h
            left = center_x - w // 2
            right = left + w
    ...
    with torch.no_grad():
        with torch.autocast(device):
            ...
            # Check and print the shape of vid
            print(f"Video shape: {vid.shape}")

            # Save the video as MP4 using ffmpeg
            video_path = os.path.join(output_folder, f"{base_count:06d}.mp4")
            writer = imageio.get_writer(video_path, fps=10, codec='libx264')  # Adjust FPS as needed
            for frame in vid:
